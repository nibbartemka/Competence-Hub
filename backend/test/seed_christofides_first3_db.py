...
CAN_OPERATION_REF_BY_KEY: dict[str, str] = {
    ...
    "build_basic_adjacency_matrix": "graph.operation.build_adjacency_matrix",
    "build_basic_incidence_matrix": "graph.operation.build_incidence_matrix",
    "estimate_basic_graph_size": "graph.operation.build_degree_sequence",
    "identify_arc_ends": "graph.operation.build_incidence_matrix",
    "count_semidegrees": "graph.operation.build_degree_sequence",
    "build_directed_incidence_matrix": "graph.operation.build_incidence_matrix",
    "compute_directed_degree_sequence": "graph.operation.build_degree_sequence",
    "trace_directed_walk": "graph.operation.build_adjacency_matrix",
    "check_bipartition": "graph.operation.build_adjacency_matrix",
    "classify_graph_type": "graph.operation.build_adjacency_matrix",
    "build_bipartite_adjacency_matrix": "graph.operation.build_adjacency_matrix",
    "compare_graph_models": "graph.operation.build_incidence_matrix",
    "compute_vertex_degrees": "graph.operation.build_degree_sequence",
}
...
TOPIC_SPECS: tuple[TopicSpec, ...] = (
    TopicSpec(
        ...
        formed=(
            ...
            ElementSpec(
                "build_basic_adjacency_matrix",
                "Строить матрицу смежности базового графа",
                "Умение строить матрицу смежности по множеству вершин и ребер базового графа.",
                CompetenceType.CAN,
            ),
            ElementSpec(
                "build_basic_incidence_matrix",
                "Строить матрицу инцидентности базового графа",
                "Умение строить матрицу инцидентности по вершинам и ребрам базового графа.",
                CompetenceType.CAN,
            ),
            ElementSpec(
                "estimate_basic_graph_size",
                "Определять порядок и размер графа",
                "Умение вычислять число вершин и число ребер по описанию графа.",
                CompetenceType.CAN,
            ),
            ElementSpec(
    ...
    TopicSpec(
        ...
        formed=(
            ...
            ElementSpec(
                "build_directed_incidence_matrix",
                "Строить матрицу инцидентности орграфа",
                "Умение представлять ориентированный граф в виде матрицы инцидентности.",
                CompetenceType.CAN,
            ),
            ElementSpec(
                "compute_directed_degree_sequence",
                "Вычислять входящие и исходящие степени",
                "Умение вычислять для каждой вершины полустепени захода и исхода.",
                CompetenceType.CAN,
            ),
            ElementSpec(
                "trace_directed_walk",
                "Прослеживать ориентированный путь",
                "Умение проверять, что последовательность дуг образует корректный ориентированный путь.",
                CompetenceType.CAN,
            ),
            ElementSpec(
                "analyze_directions",
    ...
    TopicSpec(
        ...
        formed=(
            ...
            ElementSpec(
                "build_bipartite_adjacency_matrix",
                "Строить матрицу смежности двудольного графа",
                "Умение строить матрицу смежности для двудольного или простого неориентированного графа.",
                CompetenceType.CAN,
            ),
            ElementSpec(
                "compare_graph_models",
                "Сравнивать простой граф, мультиграф и псевдограф",
                "Умение различать графовые модели по наличию петель, кратных ребер и структуры связей.",
                CompetenceType.CAN,
            ),
            ElementSpec(
                "compute_vertex_degrees",
                "Вычислять степени вершин неориентированного графа",
                "Умение находить степени вершин и использовать их для распознавания регулярности графа.",
                CompetenceType.CAN,
            ),
            ElementSpec(
                "choose_graph_type",
...
def add_element_relations(
    ...
    relation_specs: list[tuple[str, str, KnowledgeElementRelationType, str]] = [
        ...
        ("build_basic_adjacency_matrix", "graph", KnowledgeElementRelationType.IMPLEMENTS, "Умение опирается на понятие графа."),
        ("build_basic_adjacency_matrix", "vertex", KnowledgeElementRelationType.IMPLEMENTS, "Умение использует вершины графа."),
        ("build_basic_adjacency_matrix", "edge", KnowledgeElementRelationType.IMPLEMENTS, "Умение использует ребра графа."),
        ("build_basic_adjacency_matrix", "adjacency", KnowledgeElementRelationType.IMPLEMENTS, "Умение выражает смежность через матрицу."),
        ("build_basic_incidence_matrix", "graph", KnowledgeElementRelationType.IMPLEMENTS, "Умение опирается на общее описание графа."),
        ("build_basic_incidence_matrix", "vertex", KnowledgeElementRelationType.IMPLEMENTS, "Умение использует множество вершин."),
        ("build_basic_incidence_matrix", "edge", KnowledgeElementRelationType.IMPLEMENTS, "Умение использует множество ребер."),
        ("build_basic_incidence_matrix", "incidence", KnowledgeElementRelationType.IMPLEMENTS, "Умение выражает инцидентность через матрицу."),
        ("estimate_basic_graph_size", "graph", KnowledgeElementRelationType.IMPLEMENTS, "Умение опирается на модель графа."),
        ("estimate_basic_graph_size", "graph_order", KnowledgeElementRelationType.IMPLEMENTS, "Умение вычисляет порядок графа."),
        ("estimate_basic_graph_size", "graph_size", KnowledgeElementRelationType.IMPLEMENTS, "Умение вычисляет размер графа."),
        ...
        ("build_directed_incidence_matrix", "digraph", KnowledgeElementRelationType.IMPLEMENTS, "Умение опирается на понятие ориентированного графа."),
        ("build_directed_incidence_matrix", "arc", KnowledgeElementRelationType.IMPLEMENTS, "Умение использует дуги орграфа."),
        ("build_directed_incidence_matrix", "direction", KnowledgeElementRelationType.IMPLEMENTS, "Умение учитывает направление связей."),
        ("compute_directed_degree_sequence", "vertex", KnowledgeElementRelationType.IMPLEMENTS, "Умение вычисляет характеристики вершин."),
        ("compute_directed_degree_sequence", "indegree", KnowledgeElementRelationType.IMPLEMENTS, "Умение использует полустепень захода."),
        ("compute_directed_degree_sequence", "outdegree", KnowledgeElementRelationType.IMPLEMENTS, "Умение использует полустепень исхода."),
        ("trace_directed_walk", "directed_path", KnowledgeElementRelationType.IMPLEMENTS, "Умение опирается на понятие ориентированного пути."),
        ("trace_directed_walk", "arc", KnowledgeElementRelationType.IMPLEMENTS, "Умение использует последовательность дуг."),
        ("trace_directed_walk", "direction", KnowledgeElementRelationType.IMPLEMENTS, "Умение проверяет согласованность направлений."),
        ...
        ("build_bipartite_adjacency_matrix", "undirected_graph", KnowledgeElementRelationType.IMPLEMENTS, "Умение опирается на неориентированный граф."),
        ("build_bipartite_adjacency_matrix", "bipartite_graph", KnowledgeElementRelationType.IMPLEMENTS, "Умение опирается на понятие двудольного графа."),
        ("build_bipartite_adjacency_matrix", "adjacency", KnowledgeElementRelationType.IMPLEMENTS, "Умение выражает смежность матрично."),
        ("compare_graph_models", "simple_graph", KnowledgeElementRelationType.IMPLEMENTS, "Умение различает простой граф."),
        ("compare_graph_models", "multigraph", KnowledgeElementRelationType.IMPLEMENTS, "Умение различает мультиграф."),
        ("compare_graph_models", "pseudograph", KnowledgeElementRelationType.IMPLEMENTS, "Умение различает псевдограф."),
        ("compute_vertex_degrees", "vertex", KnowledgeElementRelationType.IMPLEMENTS, "Умение использует вершины графа."),
        ("compute_vertex_degrees", "vertex_degree", KnowledgeElementRelationType.IMPLEMENTS, "Умение вычисляет степень вершины."),
        ("compute_vertex_degrees", "regular_graph", KnowledgeElementRelationType.IMPLEMENTS, "Умение использует степени для распознавания регулярности."),
