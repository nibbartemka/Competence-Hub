...
DEFAULT_DB_PATH = Path(
    os.environ.get(
        ...
        str(ROOT_DIR / "app_christofides_chapters_1_3_v2.db"),
    )
)
...
TOPIC_SPECS: tuple[TopicSpec, ...] = (
    ...
    TopicSpec(
        ...
        formed=(
            ...
            ElementSpec(
                ...
                "Строить матрицу достижимости графа",
                "Умение по ориентированному графу строить матрицу достижимости.",
                competence_type=CompetenceType.CAN,
                operation_ref="graph.operation.build_reachability_matrix",
                threshold=76,
            ),
            ElementSpec(
                "find_strong_components",
                "Выделять сильные компоненты графа",
                "Умение по ориентированному графу выделять его сильные компоненты.",
                competence_type=CompetenceType.CAN,
                operation_ref="graph.operation.find_strong_components",
                threshold=78,
            ),
            ElementSpec(
                "find_graph_basis",
                "Находить базу графа",
                "Умение по ориентированному графу находить его базу через источниковые сильные компоненты.",
                competence_type=CompetenceType.CAN,
                operation_ref="graph.operation.find_graph_basis",
                threshold=80,
            ),
            ElementSpec(
    ...
    TopicSpec(
        ...
        formed=(
            ...
            ElementSpec(
                "solve_minimum_cover",
                "Решать задачу о наименьшем покрытии графа",
                "Умение находить покрытие вершин минимальной мощности для заданного графа.",
                competence_type=CompetenceType.CAN,
                operation_ref="graph.operation.solve_minimum_cover",
                threshold=78,
            ),
            ElementSpec(
                "find_domination_number",
                "Определять число доминирования графа",
                "Умение находить число доминирования и предъявлять наименьшее доминирующее множество.",
                competence_type=CompetenceType.CAN,
                operation_ref="graph.operation.find_domination_number",
                threshold=79,
            ),
            ElementSpec(
                "find_independent_vertex_set",
                "Находить независимое множество вершин графа",
                "Умение находить независимое множество вершин максимальной мощности для заданного графа.",
                competence_type=CompetenceType.CAN,
                operation_ref="graph.operation.find_independent_vertex_set",
                threshold=80,
            ),
            ElementSpec(
...
CAN_TASK_SPECS: dict[str, dict[str, Any]] = {
    ...
    "build_reachability_matrix": {
        "title": "Уметь: матрица достижимости",
        "prompt": (
            "Построй матрицу достижимости ориентированного графа A, B, C, D с дугами "
            "A->B, B->C, A->D, D->C."
        ),
        "difficulty": 46,
        "related_keys": ["reachability", "reachability_matrix", "transitive_closure"],
        "relation_specs": [
            ("build_reachability_matrix", "reachability", KnowledgeElementRelationType.BUILDS_ON),
            ("build_reachability_matrix", "reachability_matrix", KnowledgeElementRelationType.IMPLEMENTS),
        ],
        "input_payload": {
            "vertices": ["A", "B", "C", "D"],
            "edges": [
    ...
    "find_strong_components": {
        "title": "Уметь: сильные компоненты графа",
        "prompt": (
            "Выдели сильные компоненты ориентированного графа A, B, C, D, E с дугами "
            "A->B, B->A, B->C, C->D, D->C, D->E."
        ),
        "difficulty": 48,
        "related_keys": ["strong_component", "reachability", "reachability_matrix"],
        "relation_specs": [
            ("find_strong_components", "reachability", KnowledgeElementRelationType.BUILDS_ON),
            ("find_strong_components", "strong_component", KnowledgeElementRelationType.IMPLEMENTS),
        ],
        "input_payload": {
            "vertices": ["A", "B", "C", "D", "E"],
            "edges": [
    ...
    "find_graph_basis": {
        "title": "Уметь: база графа",
        "prompt": (
            "Найди базу ориентированного графа A, B, C, D, E с дугами "
            "A->B, B->A, B->C, D->C, C->E."
        ),
        "difficulty": 50,
        "related_keys": ["basis", "strong_component", "reachability"],
        "relation_specs": [
            ("find_graph_basis", "strong_component", KnowledgeElementRelationType.BUILDS_ON),
            ("find_graph_basis", "basis", KnowledgeElementRelationType.IMPLEMENTS),
        ],
        "input_payload": {
            "vertices": ["A", "B", "C", "D", "E"],
            "edges": [
    ...
    "solve_minimum_cover": {
        "title": "Уметь: задача о наименьшем покрытии",
        "prompt": (
            "Найди покрытие минимальной мощности для графа A, B, C, D, E с ребрами A-B, A-C, B-C, C-D, D-E."
        ),
        "difficulty": 54,
        "related_keys": ["covering_problem", "independent_set", "maximum_independent_set"],
        "relation_specs": [
            ("solve_minimum_cover", "covering_problem", KnowledgeElementRelationType.IMPLEMENTS),
            ("solve_minimum_cover", "maximum_independent_set", KnowledgeElementRelationType.RELIES_ON),
        ],
        "input_payload": {
            "vertices": ["A", "B", "C", "D", "E"],
            "edges": [
    ...
    "find_domination_number": {
        "title": "Уметь: число доминирования",
        "prompt": (
            "Найди число доминирования графа A, B, C, D, E с ребрами A-B, B-C, C-D, D-E."
        ),
        "difficulty": 56,
        "related_keys": ["dominating_set", "minimum_dominating_set", "domination_number"],
        "relation_specs": [
            ("find_domination_number", "minimum_dominating_set", KnowledgeElementRelationType.BUILDS_ON),
            ("find_domination_number", "domination_number", KnowledgeElementRelationType.IMPLEMENTS),
        ],
        "input_payload": {
            "vertices": ["A", "B", "C", "D", "E"],
            "edges": [
    ...
    "find_independent_vertex_set": {
        "title": "Уметь: независимое множество вершин",
        "prompt": (
            "Найди независимое множество максимальной мощности для графа A, B, C, D, E с ребрами A-B, B-C, C-D, D-E, A-E."
        ),
        "difficulty": 58,
        "related_keys": ["independent_set", "maximum_independent_set", "independence_number"],
        "relation_specs": [
            ("find_independent_vertex_set", "independent_set", KnowledgeElementRelationType.IMPLEMENTS),
            ("find_independent_vertex_set", "maximum_independent_set", KnowledgeElementRelationType.BUILDS_ON),
        ],
        "input_payload": {
            "vertices": ["A", "B", "C", "D", "E"],
            "edges": [
...
MASTER_TASK_SPECS: dict[str, dict[str, Any]] = {
    "analyze_intro_graph_problem": {
        ...
        "difficulty": 64,
        "related_keys": ["graph", "vertex", "edge", "adjacency_matrix", "incidence_matrix", "degree"],
        "relation_specs": [
            ...
            ("analyze_intro_graph_problem", "graph", KnowledgeElementRelationType.RELIES_ON),
            ("analyze_intro_graph_problem", "vertex", KnowledgeElementRelationType.RELIES_ON),
            ("analyze_intro_graph_problem", "edge", KnowledgeElementRelationType.RELIES_ON),
            ("analyze_intro_graph_problem", "adjacency_matrix", KnowledgeElementRelationType.RELIES_ON),
            ("analyze_intro_graph_problem", "incidence_matrix", KnowledgeElementRelationType.RELIES_ON),
            ("analyze_intro_graph_problem", "degree", KnowledgeElementRelationType.RELIES_ON),
        ],
    ...
    "analyze_organization_basis": {
        ...
        "difficulty": 68,
        "related_keys": ["reachability_matrix", "strong_component", "basis"],
        "relation_specs": [
            ("analyze_organization_basis", "build_reachability_matrix", KnowledgeElementRelationType.AUTOMATES),
            ("analyze_organization_basis", "find_strong_components", KnowledgeElementRelationType.AUTOMATES),
            ("analyze_organization_basis", "find_graph_basis", KnowledgeElementRelationType.AUTOMATES),
            ("analyze_organization_basis", "reachability_matrix", KnowledgeElementRelationType.RELIES_ON),
            ("analyze_organization_basis", "strong_component", KnowledgeElementRelationType.RELIES_ON),
            ("analyze_organization_basis", "basis", KnowledgeElementRelationType.RELIES_ON),
        ],
    ...
    "justify_minimum_cover": {
        ...
        "difficulty": 72,
        "related_keys": ["covering_problem"],
        "relation_specs": [
            ("justify_minimum_cover", "solve_minimum_cover", KnowledgeElementRelationType.AUTOMATES),
            ("justify_minimum_cover", "covering_problem", KnowledgeElementRelationType.RELIES_ON),
        ],
        "placeholder": "Опишите сведение к покрытию и обоснование минимальности выбранного решения.",
    },
}

MASTER_DOMAIN_OBJECT_KEYS: dict[str, tuple[str, ...]] = {
    "analyze_intro_graph_problem": (
        "graph",
        "vertex",
        "edge",
    ...
    "justify_minimum_cover": ("covering_problem",),
}
...
def seed_knowledge_graph(
    ...
    add_master_domain_objects(session, elements)
    session.flush()
    return topics, elements


def add_master_domain_objects(
    session: Session,
    elements: dict[str, KnowledgeElement],
) -> None:
    for master_key, knowledge_keys in MASTER_DOMAIN_OBJECT_KEYS.items():
        master_element = elements[master_key]
        for knowledge_key in knowledge_keys:
            knowledge_element = elements[knowledge_key]
            session.add(
                MasterElementDomainObject(
                    object_name=knowledge_element.name,
                    master_element_id=master_element.id,
                    knowledge_element_id=knowledge_element.id,
                )
            )
...
def add_element_relations(
    ...
    relation_specs: list[tuple[str, str, KnowledgeElementRelationType, str]] = [
        ...
        ("analyze_intro_graph_problem", "graph", KnowledgeElementRelationType.RELIES_ON, "\u0412\u043b\u0430\u0434\u0435\u043d\u0438\u0435 \u043e\u043f\u0438\u0440\u0430\u0435\u0442\u0441\u044f \u043d\u0430 \u043c\u043e\u0434\u0435\u043b\u044c \u0433\u0440\u0430\u0444\u0430."),
        ("analyze_intro_graph_problem", "vertex", KnowledgeElementRelationType.RELIES_ON, "\u0412\u043b\u0430\u0434\u0435\u043d\u0438\u0435 \u043e\u043f\u0438\u0440\u0430\u0435\u0442\u0441\u044f \u043d\u0430 \u043f\u043e\u043d\u044f\u0442\u0438\u0435 \u0432\u0435\u0440\u0448\u0438\u043d\u044b."),
        ("analyze_intro_graph_problem", "edge", KnowledgeElementRelationType.RELIES_ON, "\u0412\u043b\u0430\u0434\u0435\u043d\u0438\u0435 \u043e\u043f\u0438\u0440\u0430\u0435\u0442\u0441\u044f \u043d\u0430 \u043f\u043e\u043d\u044f\u0442\u0438\u0435 \u0440\u0435\u0431\u0440\u0430."),
        ("analyze_intro_graph_problem", "adjacency_matrix", KnowledgeElementRelationType.RELIES_ON, "\u0412\u043b\u0430\u0434\u0435\u043d\u0438\u0435 \u043e\u043f\u0438\u0440\u0430\u0435\u0442\u0441\u044f \u043d\u0430 \u043c\u0430\u0442\u0440\u0438\u0446\u0443 \u0441\u043c\u0435\u0436\u043d\u043e\u0441\u0442\u0438 \u043f\u0440\u0438 \u0430\u043d\u0430\u043b\u0438\u0437\u0435 \u0433\u0440\u0430\u0444\u0430."),
        ("analyze_intro_graph_problem", "incidence_matrix", KnowledgeElementRelationType.RELIES_ON, "\u0412\u043b\u0430\u0434\u0435\u043d\u0438\u0435 \u043e\u043f\u0438\u0440\u0430\u0435\u0442\u0441\u044f \u043d\u0430 \u043c\u0430\u0442\u0440\u0438\u0446\u0443 \u0438\u043d\u0446\u0438\u0434\u0435\u043d\u0446\u0438\u0439 \u043f\u0440\u0438 \u0430\u043d\u0430\u043b\u0438\u0437\u0435 \u0433\u0440\u0430\u0444\u0430."),
        ("analyze_intro_graph_problem", "degree", KnowledgeElementRelationType.RELIES_ON, "\u0412\u043b\u0430\u0434\u0435\u043d\u0438\u0435 \u043e\u043f\u0438\u0440\u0430\u0435\u0442\u0441\u044f \u043d\u0430 \u0441\u0442\u0435\u043f\u0435\u043d\u0438 \u0432\u0435\u0440\u0448\u0438\u043d \u043f\u0440\u0438 \u0440\u0430\u0437\u0431\u043e\u0440\u0435 \u0433\u0440\u0430\u0444\u0430."),
        ...
        ("build_reachability_matrix", "reachability", KnowledgeElementRelationType.BUILDS_ON, "Умение опирается на понятие достижимости."),
        ("build_reachability_matrix", "reachability_matrix", KnowledgeElementRelationType.IMPLEMENTS, "Умение выражает достижимость матрицей."),
        ("find_strong_components", "reachability", KnowledgeElementRelationType.BUILDS_ON, "Умение использует взаимную достижимость вершин."),
        ("find_strong_components", "strong_component", KnowledgeElementRelationType.IMPLEMENTS, "Умение выделяет сильные компоненты графа."),
        ("find_graph_basis", "strong_component", KnowledgeElementRelationType.BUILDS_ON, "Умение использует разбиение на сильные компоненты."),
        ("find_graph_basis", "basis", KnowledgeElementRelationType.IMPLEMENTS, "Умение находит базу графа."),
        ("analyze_organization_basis", "build_reachability_matrix", KnowledgeElementRelationType.AUTOMATES, "\u0412\u043b\u0430\u0434\u0435\u043d\u0438\u0435 \u0432\u043a\u043b\u044e\u0447\u0430\u0435\u0442 \u043f\u043e\u0441\u0442\u0440\u043e\u0435\u043d\u0438\u0435 \u043c\u0430\u0442\u0440\u0438\u0446\u044b \u0434\u043e\u0441\u0442\u0438\u0436\u0438\u043c\u043e\u0441\u0442\u0438."),
        ("analyze_organization_basis", "find_strong_components", KnowledgeElementRelationType.AUTOMATES, "\u0412\u043b\u0430\u0434\u0435\u043d\u0438\u0435 \u0432\u043a\u043b\u044e\u0447\u0430\u0435\u0442 \u0432\u044b\u0434\u0435\u043b\u0435\u043d\u0438\u0435 \u0441\u0438\u043b\u044c\u043d\u044b\u0445 \u043a\u043e\u043c\u043f\u043e\u043d\u0435\u043d\u0442."),
        ("analyze_organization_basis", "find_graph_basis", KnowledgeElementRelationType.AUTOMATES, "\u0412\u043b\u0430\u0434\u0435\u043d\u0438\u0435 \u0432\u043a\u043b\u044e\u0447\u0430\u0435\u0442 \u043d\u0430\u0445\u043e\u0436\u0434\u0435\u043d\u0438\u0435 \u0431\u0430\u0437\u044b \u0433\u0440\u0430\u0444\u0430."),
        ("analyze_organization_basis", "reachability_matrix", KnowledgeElementRelationType.RELIES_ON, "\u0412\u043b\u0430\u0434\u0435\u043d\u0438\u0435 \u043e\u043f\u0438\u0440\u0430\u0435\u0442\u0441\u044f \u043d\u0430 \u043c\u0430\u0442\u0440\u0438\u0446\u0443 \u0434\u043e\u0441\u0442\u0438\u0436\u0438\u043c\u043e\u0441\u0442\u0438 \u043f\u0440\u0438 \u0430\u043d\u0430\u043b\u0438\u0437\u0435 \u0433\u0440\u0430\u0444\u0430."),
        ("analyze_organization_basis", "strong_component", KnowledgeElementRelationType.RELIES_ON, "\u0412\u043b\u0430\u0434\u0435\u043d\u0438\u0435 \u043e\u043f\u0438\u0440\u0430\u0435\u0442\u0441\u044f \u043d\u0430 \u0432\u044b\u0434\u0435\u043b\u0435\u043d\u0438\u0435 \u0441\u0438\u043b\u044c\u043d\u044b\u0445 \u043a\u043e\u043c\u043f\u043e\u043d\u0435\u043d\u0442."),
        ("analyze_organization_basis", "basis", KnowledgeElementRelationType.RELIES_ON, "\u0412\u043b\u0430\u0434\u0435\u043d\u0438\u0435 \u043e\u043f\u0438\u0440\u0430\u0435\u0442\u0441\u044f \u043d\u0430 \u043f\u043e\u043d\u044f\u0442\u0438\u0435 \u0431\u0430\u0437\u044b \u0433\u0440\u0430\u0444\u0430."),
        ...
        ("solve_minimum_cover", "covering_problem", KnowledgeElementRelationType.IMPLEMENTS, "Умение решает задачу о наименьшем покрытии."),
        ("solve_minimum_cover", "maximum_independent_set", KnowledgeElementRelationType.RELIES_ON, "Умение использует связь покрытия с наибольшим независимым множеством."),
        ("find_domination_number", "minimum_dominating_set", KnowledgeElementRelationType.BUILDS_ON, "Умение опирается на понятие наименьшего доминирующего множества."),
        ("find_domination_number", "domination_number", KnowledgeElementRelationType.IMPLEMENTS, "Умение определяет число доминирования графа."),
        ("find_independent_vertex_set", "independent_set", KnowledgeElementRelationType.IMPLEMENTS, "Умение выделяет независимое множество вершин."),
        ("find_independent_vertex_set", "maximum_independent_set", KnowledgeElementRelationType.BUILDS_ON, "Умение ищет независимое множество максимальной мощности."),
        ("justify_minimum_cover", "solve_minimum_cover", KnowledgeElementRelationType.AUTOMATES, "\u0412\u043b\u0430\u0434\u0435\u043d\u0438\u0435 \u0432\u043a\u043b\u044e\u0447\u0430\u0435\u0442 \u0440\u0435\u0448\u0435\u043d\u0438\u0435 \u0437\u0430\u0434\u0430\u0447\u0438 \u043e \u043d\u0430\u0438\u043c\u0435\u043d\u044c\u0448\u0435\u043c \u043f\u043e\u043a\u0440\u044b\u0442\u0438\u0438."),
        ("justify_minimum_cover", "covering_problem", KnowledgeElementRelationType.RELIES_ON, "\u0412\u043b\u0430\u0434\u0435\u043d\u0438\u0435 \u043e\u043f\u0438\u0440\u0430\u0435\u0442\u0441\u044f \u043d\u0430 \u0437\u0430\u0434\u0430\u0447\u0443 \u043e \u043d\u0430\u0438\u043c\u0435\u043d\u044c\u0448\u0435\u043c \u043f\u043e\u043a\u0440\u044b\u0442\u0438\u0438."),
    ]
...
def seed_student_mastery_and_progress(
    ...
    create_progress_with_attempt(
        ...
        task=group_2_trajectory.tasks[f"can:build_reachability_matrix:{group_2_trajectory.trajectory.id}"],
...
def print_seed_summary(
    ...
    print("\n=== Christofides chapters 1-3 seed v2 ===")
